...
## 2. 연구실 GPU 노드 연동 (RTX 6000)

백엔드에서 연구실에 위치한 RTX 6000 GPU의 실시간 자원 정보를 수집하기 위해 SSH 접속 정보 및 워커 주소를 설정합니다.

### SSH 모니터링 직접 접속 테스트 (비밀번호 인증)
```powershell
# 22345 포트를 통한 연구실 GPU 접속 확인
ssh sslab@155.230.118.52 -p 22345
# (비밀번호 요구 시: sslab1!2 입력)
```

### 백엔드 환경변수 설정 (`Project/Infra/docker-compose.yaml`)
백엔드가 컨테이너 내부에서 연구실 SSH에 접근할 수 있도록 다음 환경 변수들이 지정되어 있습니다.
- `RTX6000_SSH_HOST=155.230.118.52`
- `RTX6000_SSH_PORT=22345`
- `RTX6000_SSH_USER=sslab`
- `RTX6000_SSH_PASSWORD=sslab1!2`

### 2.2. 포트 포워딩 (SSH 터널링)을 통한 Worker API 연동
연구실 서버 내부에서 돌고 있는 GPU Worker API(5000번 포트)를 로컬 PC의 `5001` 포트로 포워딩하여 백엔드가 메트릭을 수집하도록 연결합니다.

*터미널에서 다음 명령어를 실행하여 터널링을 유지합니다:*
```powershell
# 연구실 서버의 내부 5000번 포트를 로컬의 5001 포트로 포워딩
ssh -L 5001:localhost:5000 sslab@155.230.118.52 -p 22345 -N
```
> 비밀번호 입력 후 터미널 창을 그대로 유지해야 실시간 GPU 사용량 모니터링이 가능합니다.
...
```powershell
Invoke-RestMethod -Method Post -Uri "http://localhost:8000/scheduler/analyze" -ContentType "application/json" -Body '{"prompt":"ResNet50 모델 학습하고 싶어. 배치 크기는 32로 설정해줘."}'
```
*결과값으로 최적 GPU 추천 정보와 예상 대기/연산 시간이 JSON으로 반환되는지 확인합니다.*
